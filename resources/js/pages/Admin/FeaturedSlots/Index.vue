  ...
  <AppLayout>
    ...
    <div class="mx-auto max-w-7xl px-4 py-12 sm:px-6 lg:px-8">
      ...
      <div class="mb-8 flex items-center justify-between">
        <div>
          <h1 class="text-3xl font-bold text-white">Featured Slots</h1>
          <p class="mt-2 text-zinc-400">Manage homepage hero and pick of the week</p>
        </div>
        <Link
      ...
      <div class="overflow-hidden rounded-lg bg-zinc-900">
        <table class="min-w-full divide-y divide-zinc-800">
          ...
          <tbody class="divide-y divide-zinc-800">
            <tr v-for="slot in slots.data" :key="slot.id" class="hover:bg-zinc-800/50">
              <td class="px-6 py-4">
                <div class="flex items-center gap-3">
                  ...
                  <div class="font-medium text-white">{{ slot.movie?.title }}</div>
                </div>
              </td>
              <td class="px-6 py-4">
                <span
              ...
              <td class="px-6 py-4 text-sm text-zinc-300">
                {{ slot.ends_at ? formatDate(slot.ends_at) : 'No end date' }}
              </td>
              <td class="px-6 py-4">
                <span
                  :class="[
      ...
      <!-- Pagination -->
      <div v-if="slots.last_page > 1" class="mt-6 flex justify-center gap-2">
        <Link
